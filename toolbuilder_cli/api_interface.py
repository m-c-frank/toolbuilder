...
def process_template(template_content, dir_tree, requested_file_path):
    return template_content.replace("<REPO_DIR_TREE>", dir_tree).replace(
        "<REQUESTED_FILENAME>", requested_file_path
    )


def send_request(request_msg, function_name, processed_template=None):
    context_source = FUNCTION_CONTEXTS.get(
...
if __name__ == "__main__":
    ...
    content = fetch_repository_content("toolbuilder", dir_tree, requested_file_path)
