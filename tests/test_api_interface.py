    ...
    "choices": [
        {
            "finish_reason": "stop",
            "index": 0,
            "message": {"content": "Sample Response, BFS", "role": "assistant"},
        }
    ]
}
...
def test_send_request_general(mocker):
    mocker.patch("neuralapi.ChatCompletion.create", return_value=sample_response)
    result = api_interface.send_request(
        "World Series 2020 location?", "general_request"
    )
...
def test_send_request_with_template(mocker):
    mocker.patch("neuralapi.ChatCompletion.create", return_value=sample_response)
    mocker.patch(
        "toolbuilder_cli.api_interface.load_prompt_template",
        return_value="<REPO_DIR_TREE> <REQUESTED_FILENAME>",
    )

    result = api_interface.send_request(
        "World Series 2020 location?", "fetch_repository_content", "mock_template"
    )
    assert result == "Sample Response, BFS"
...
def test_fetch_repository_content(mocker):
    ...
    content = api_interface.fetch_repository_content(
        "toolbuilder", "mock_dir_tree", "mock_target_file_name"
    )
